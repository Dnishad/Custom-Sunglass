...
function threeGlassFace(spec) {
  ...
  new THREE.BufferGeometryLoader().load(spec.lensesMeshURL, function (glassesLensesGeometry) {
    ...
    mesh.add(glassesLensesMesh);
    // obj2 = glassesLensesMesh;
    window.debugMatLens = mat; // to debug the material il the JS console
  });

  const occluderMesh = new THREE.Mesh();
      new THREE.BufferGeometryLoader().load(spec.occluderURL, function(occluderGeometry){
        const mat = new THREE.ShaderMaterial({
...
function init_threeScene(spec) {
  // threeStuffs = objinit(spec, detect_callback);
  ...
  const r = threeGlassFace({
    envMapURL: "sky.jpg",
    frameMeshURL: "assets/glassesFramesBranchesBent.json",
    lensesMeshURL: "assets/glassesLenses.json",
    occluderURL: "assets/face.json"
  });
  faceObject.add(r.Face);
  // console.log(threeStuffs.faceObject);

  r.Face.rotation.set(0.3, 0, 0);
  r.Face.position.set(0, 0.1, -0.04);
  r.Face.scale.multiplyScalar(0.0084);
  // console.log(r.occluder);
  threeGlasses = r.FrameGlass;
  threeGlasses.position.set(0, 0.07, 0.4);
  threeGlasses.scale.multiplyScalar(0.006);
  faceObject.add(threeGlasses);
  // scene.add(faceObject)
  // console.log(r.glasses);
...
function intdetect(detectState) {
...
function update_positions3D(ds, camera, _scaleW) {
...
function render(detectState, camera, _scaleW) {
  const ds = (_isMultiFaces) ? detectState : [detectState];
